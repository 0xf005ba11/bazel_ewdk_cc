...
# System env vars that Windows itself contributes (cmd.exe, powershell.exe,
# script tools all rely on these to function). SetupBuildEnv.cmd does not set
# them, so a strict diff between pre-setup and post-setup env would drop them.
# We forward these as-is from the launching shell so build actions can still
# execute cmd/PowerShell/script tools; everything else the launching shell had
# in scope (PSMODULEPATH from pwsh, language/tool version-manager vars, etc.)
# is discarded so action envs stay consistent across developer machines.
...
def _get_ewdk_env(repository_ctx, ewdkdir, host_cpu):
    """Retrieve the envvars contributed by the EWDK's SetupBuildEnv.cmd.

    Captures the environment both before and after invoking SetupBuildEnv.cmd
    / SetupVSEnv.cmd and returns only the keys those scripts actually add or
    modify, plus a fixed allowlist of Windows system env vars every action
    needs to function. Anything else the launching shell had in scope (module
    paths from pwsh, tool-installer state, personal PATH additions, etc.) is
    dropped so action environments stay hermetic across developer machines.
    """
...
::
:: Dump the env BEFORE SetupBuildEnv runs so the caller can diff pre- vs
:: post-setup and keep only the keys the EWDK actually contributes.
::
echo ===BAZEL_EWDK_BEFORE===
set
echo ===BAZEL_EWDK_END===
...
::
:: Dump the environment variables AFTER setup for diffing.
::
echo ===BAZEL_EWDK_AFTER===
set
echo ===BAZEL_EWDK_END===
""".format(ewdkdir, host_arch, tgt_arch, platform_arch, bin_arch)
    ...
    # Sections are delimited by sentinel lines echoed from the batch above.
    # `dir /b tlbexp.exe /s` output lands outside any env section, so we
    # scan for tlbexp lines while `section` is None.
    section = None
    for raw in envs.split("\n"):
        line = raw.strip()
        if line == "===BAZEL_EWDK_BEFORE===":
            section = before
            continue
        if line == "===BAZEL_EWDK_AFTER===":
            section = after
            continue
        if line == "===BAZEL_EWDK_END===":
            section = None
            continue
        offset = line.find("=")
        if offset == -1:
            ...
            continue
        if section != None:
            section[line[:offset].upper()] = line[offset + 1:]

    env_map = {}
    for key, value in after.items():
        if key == "PATH":
            # PATH is prepended by SetupBuildEnv, but the tail is still the
            # launching shell's PATH. Strip that tail when it's present as a
            # suffix so we don't leak the developer's user PATH (custom tool
            # installs, pwsh 7 shims, version-manager shims, etc.) into every
            # action. The rewritten value falls through to the diff check
            # below and is retained because SetupBuildEnv always changes PATH.
            prev = before.get(key, "")
            if prev and value.endswith(";" + prev):
                value = value[:-(len(prev) + 1)]
        if key in _SYSTEM_ENV_ALLOWLIST or before.get(key) != value:
            env_map[key] = value
    ...
    static_link_msvcrt_feature = feature(
        ...
        flag_sets = [
            flag_set(
                ...
                with_features = [with_feature_set(not_features = ["dbg", "disable_msvcrt", "static_link_msvcrt_no_debug"])],
            ),
            flag_set(
                actions = [ACTION_NAMES.c_compile, ACTION_NAMES.cpp_compile],
    ...
    dynamic_link_msvcrt_feature = feature(
        ...
        flag_sets = [
            flag_set(
                ...
                with_features = [with_feature_set(not_features = ["dbg", "static_link_msvcrt", "disable_msvcrt", "static_link_msvcrt_no_debug", "dynamic_link_msvcrt_no_debug"])],
            ),
            flag_set(
                actions = [ACTION_NAMES.c_compile, ACTION_NAMES.cpp_compile],
                flag_groups = [flag_group(flags = ["/MDd"])],
                with_features = [with_feature_set(features = ["dbg"], not_features = ["static_link_msvcrt", "disable_msvcrt", "static_link_msvcrt_no_debug", "dynamic_link_msvcrt_no_debug"])],
            ),
            flag_set(
                actions = all_link_actions,
    ...
    dynamic_link_msvcrt_no_debug_feature = feature(
        ...
        flag_sets = [
            flag_set(
                actions = [ACTION_NAMES.c_compile, ACTION_NAMES.cpp_compile],
                flag_groups = [flag_group(flags = ["/MD"])],
                with_features = [with_feature_set(not_features = ["static_link_msvcrt", "disable_msvcrt", "static_link_msvcrt_no_debug"])],
            ),
            flag_set(
                actions = all_link_actions,
                flag_groups = [flag_group(flags = ["/DEFAULTLIB:msvcrt.lib"])],
